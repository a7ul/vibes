...
function extractJsonSchema(schema: ZodTypeAny): Record<string, unknown> {
	...
	const schemaInternal = schema as unknown as { _def?: Record<string, unknown> };
...
function buildFromDef(typeName: string | undefined, def: Record<string, unknown>, schema: unknown): Record<string, unknown> {
	switch (typeName) {
		...
		case "ZodOptional": return extractJsonSchema(def.innerType as ZodTypeAny);
		case "ZodNullable": return {
			oneOf: [extractJsonSchema(def.innerType as ZodTypeAny), { type: "null" }],
		};
		case "ZodObject": {
			...
			for (const [k, v] of Object.entries(shape)) {
				...
				const isOptional = vTypeName === "ZodOptional" || vTypeName === "ZodNullable";
