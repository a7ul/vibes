...
function generateFromZodDef(zodType: ZodTypeAny): unknown {
	...
	switch (type) {
		...
		case "default": {
			...
			if (typeof defaultValue === "function") return (defaultValue as () => unknown)();
			return defaultValue;
		}
		case "object": {
 ...
 */
function generateArgsFromInputSchema(inputSchema: unknown): Record<string, unknown> {
...
function buildToolCallContent(tools: FunctionTool[]): DoGenerateResult["content"] {
	return tools.map((t) => ({
	...
	doStream(options: DoGenerateParams): Promise<Awaited<ReturnType<MockLanguageModelV3["doStream"]>>> {
		return this.doGenerate(options).then((result) => {
			...
			for (const part of result.content) {
				if (part.type === "text") {
					chunks.push({ type: "text-delta" as const, id: "text-1", delta: part.text });
