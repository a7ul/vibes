...
Deno.test("serializeRunState - throws on missing role", () => {
	const badMessages = [{ content: "hello" }] as unknown as import("ai").ModelMessage[];
	assertThrows(
...
Deno.test("serializeRunState - throws on missing content", () => {
	const badMessages = [{ role: "user" }] as unknown as import("ai").ModelMessage[];
	assertThrows(
		() => serializeRunState(badMessages),
		TypeError,
		"missing 'content'",
...
Deno.test("deserializeRunState - round trips simple messages", () => {
	...
	assertEquals((deserialized[0] as Record<string, unknown>)["content"], "Hello");
});
