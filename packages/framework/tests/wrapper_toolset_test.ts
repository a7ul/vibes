...
class RecordingWrapper<TDeps = undefined> extends WrapperToolset<TDeps> {
	readonly log: Array<{ name: string; args: Record<string, unknown>; result: unknown }> = [];
...
/** Transforms args before passing to the inner tool. */
class ArgTransformWrapper<TDeps = undefined> extends WrapperToolset<TDeps> {
	readonly capturedArgs: Record<string, unknown>[] = [];
	...
	async callTool(
		...
		const transformed = { ...args, _injected: true };
		this.capturedArgs.push(transformed);
