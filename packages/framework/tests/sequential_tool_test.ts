...
Deno.test("sequential tool - sequential tools serialize overlapping calls", async () => {
	...
	const responses = mockValues<DoGenerateResult>(
		...
		{
			content: [
				{ type: "tool-call", toolCallId: "tc1", toolName: "tool_a", input: JSON.stringify({}) },
				{ type: "tool-call", toolCallId: "tc2", toolName: "tool_b", input: JSON.stringify({}) },
			],
	...
	assertEquals(aBeforeB || bBeforeA, true, `executionOrder: ${executionOrder.join(", ")}`);
});
...
Deno.test("sequential tool - non-sequential tools are not affected by sequential mutex", async () => {
	...
	const responses = mockValues<DoGenerateResult>(
		{
			content: [
				{ type: "tool-call", toolCallId: "tc1", toolName: "slow", input: JSON.stringify({}) },
				{ type: "tool-call", toolCallId: "tc2", toolName: "fast", input: JSON.stringify({}) },
			],
			finishReason: { unified: "tool-calls" as const, raw: undefined },
			usage: {
				inputTokens: { total: 10, noCache: 10, cacheRead: 0, cacheWrite: undefined },
	...
	assertEquals(fastEnd < slowEnd, true, `fast should end before slow: ${executionLog.join(", ")}`);
});
