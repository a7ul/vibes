...
Deno.test("isBinaryContent - returns false for non-BinaryContent objects", () => {
	...
	assertEquals(isBinaryContent({ type: "text", mimeType: "text/plain" }), false);
	assertEquals(isBinaryContent({ type: "binary", mimeType: "image/png" }), false); // missing data
	assertEquals(isBinaryContent({ type: "binary", data: new Uint8Array() }), false); // missing mimeType
...
Deno.test("isUploadedFile - returns false for non-UploadedFile objects", () => {
	...
	assertEquals(isUploadedFile({ type: "binary", fileId: "x", mimeType: "a" }), false);
...
Deno.test("isMultiModalContent - returns true for both BinaryContent and UploadedFile", () => {
	const bc: BinaryContent = { type: "binary", mimeType: "image/png", data: new Uint8Array() };
	const uf: UploadedFile = { type: "uploaded_file", fileId: "x", mimeType: "text/plain" };
...
Deno.test("BinaryContent - tool can return BinaryContent without error", async () => {
	const imgTool = tool({
		...
		parameters: z.object({}),
		execute: async (): Promise<import("../binary_content.ts").BinaryContent> => ({
			type: "binary",
			mimeType: "image/png",
...
Deno.test("UploadedFile - tool can return UploadedFile without error", async () => {
	const fileTool = tool({
		...
		parameters: z.object({}),
		execute: async (): Promise<import("../binary_content.ts").UploadedFile> => ({
			type: "uploaded_file",
