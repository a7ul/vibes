...
Deno.test("Agent.override - replaces model", async () => {
	...
	const overridden = await agent.override({ model: overrideModel }).run("prompt");
...
Deno.test("Agent.override - replaces system prompt", async () => {
	...
	const model = new MockLanguageModelV3({
		doGenerate: (opts) => {
			const sys = opts.prompt.find((m: { role: string }) => m.role === "system");
...
Deno.test("Agent.override - replaces tools", async () => {
	...
	const originalTool = tool({
		...
		parameters: z.object({}),
		execute: async () => { originalCalled = true; return "original"; },
	});
	const overrideTool = tool({
		name: "do_thing",
		description: "override",
		parameters: z.object({}),
		execute: async () => { overrideCalled = true; return "override"; },
	});
