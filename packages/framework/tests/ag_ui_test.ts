...
Deno.test("AGUIAdapter - TEXT_MESSAGE_CONTENT deltas reconstruct full text", async () => {
	...
	const contents = events.filter((e) => e.type === "TEXT_MESSAGE_CONTENT") as Extract<
		AGUIEvent,
...
Deno.test("AGUIAdapter - emits TOOL_CALL_START with correct name", async () => {
	const myTool = tool({
		...
		execute: (_ctx, args: { name: string }) => Promise.resolve(`Hello ${args.name}`),
	});
