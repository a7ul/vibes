...
import { mockValues, textResponse, toolCallResponse, type DoGenerateResult } from "./_helpers.ts";
...
Deno.test("FunctionModel - receives messages", async () => {
	...
	const userMsg = capturedMessages.find((m: { role: string }) => m.role === "user");
...
Deno.test("FunctionModel - works with agent.override", async () => {
	...
	const result = await agent.override({ model: testModel }).run("What is the answer?");
