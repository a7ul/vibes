...
import { Agent, type HistoryProcessor, trimHistoryProcessor } from "../mod.ts";
import {
...
Deno.test("historyProcessors - multiple processors applied in order", async () => {
  ...
  const p1: HistoryProcessor = (msgs) => {
    log.push("p1");
    return msgs;
  };
  const p2: HistoryProcessor = (msgs) => {
    log.push("p2");
    return msgs;
  };
  const p3: HistoryProcessor = (msgs) => {
    log.push("p3");
    return msgs;
  };
...
Deno.test("Agent.addHistoryProcessor - adds processor after construction", async () => {
  let called = false;
  const processor: HistoryProcessor = (msgs) => {
    called = true;
    return msgs;
  };
