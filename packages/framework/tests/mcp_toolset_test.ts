...
import type { MCPClient, MCPTool, MCPCallResult } from "../mod.ts";
...
Deno.test("MCPToolset - exposes MCP tools to agent", async () => {
	...
	const model = new MockLanguageModelV3({
		doGenerate: (opts) => {
			capturedToolNames = (opts.tools ?? []).map((t: { name: string }) => t.name);
...
Deno.test("MCPToolset - getServerInstructions returns instructions", () => {
	...
	const toolset = new MCPToolset(client);
	assertEquals(toolset.getServerInstructions(), "Use this server for document retrieval.");
});
...
Deno.test("MCPManager - aggregates server instructions", () => {
	const clientA = new MockMCPClient({ serverInstructions: "Instructions from A" });
	const clientB = new MockMCPClient({ serverInstructions: "Instructions from B" });
