...
Deno.test("CombinedToolset - last toolset wins on name conflict", async () => {
	...
	const model = new MockLanguageModelV3({
		doGenerate: (opts) => {
			const entry = (opts.tools ?? []).find((t: ToolEntry) => t.name === "my_tool");
			capturedDesc = (entry as { name: string; description?: string })?.description;
			return Promise.resolve(textResponse("done"));
		},
	});

	const ts1 = new FunctionToolset([
		tool({ name: "my_tool", description: "first", parameters: z.object({}), execute: async () => "1" }),
	]);
	const ts2 = new FunctionToolset([
		tool({ name: "my_tool", description: "second", parameters: z.object({}), execute: async () => "2" }),
	]);
...
Deno.test("FilteredToolset - excludes tools when predicate returns false", async () => {
	...
	const adminTs = new FunctionToolset<Deps>([makeTool("delete_user") as unknown as import("../mod.ts").ToolDefinition<Deps>]);
