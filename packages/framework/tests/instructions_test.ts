...
function captureSystem(opts: { prompt: Array<{ role: string; content: unknown }> }): string | undefined {
...
Deno.test("instructions - static string is injected into system prompt", async () => {
	...
	const model = new MockLanguageModelV3({
		doGenerate: (opts) => {
			capturedSystem = captureSystem(opts as { prompt: Array<{ role: string; content: unknown }> });
...
Deno.test("instructions - dynamic function is resolved per-turn", async () => {
	...
	const model = new MockLanguageModelV3({
		doGenerate: (opts) => {
			capturedSystems.push(captureSystem(opts as { prompt: Array<{ role: string; content: unknown }> }));
