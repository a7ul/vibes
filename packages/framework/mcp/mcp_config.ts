...
export async function loadMCPConfig(configPath: string): Promise<MCPServerConfig[]> {
	...
	if (parsed !== null && typeof parsed === "object") {
		...
		if ("mcpServers" in obj && obj["mcpServers"] !== null && typeof obj["mcpServers"] === "object") {
			const servers = obj["mcpServers"] as Record<string, unknown>;
			const configs: MCPServerConfig[] = Object.entries(servers).map(([name, cfg]) => {
 ...
 */
export function createClientsFromConfig(configs: MCPServerConfig[]): MCPClient[] {
	return configs.map((cfg) => {
		...
		throw new Error(`Unknown MCP server type: ${(cfg as Record<string, unknown>)["type"]}`);
	});
}
...
function interpolateEnv(text: string): string {
	return text.replace(/\$\{([^}]+)\}/g, (_match, varName: string) => {
		...
		if (value === undefined) {
			throw new Error(`MCP config: environment variable "${varName}" is not set.`);
		}
		return value;
	});
}
...
function validateConfigs(items: unknown[]): MCPServerConfig[] {
	return items.map((item, idx) => {
		if (item === null || typeof item !== "object") {
			throw new Error(`MCP config[${idx}]: expected an object, got ${typeof item}`);
		}
		const obj = item as Record<string, unknown>;
		if (obj["type"] !== "stdio" && obj["type"] !== "http") {
			throw new Error(
				`MCP config[${idx}]: unknown type "${obj["type"]}" — expected "stdio" or "http"`,
			);
		}
		if (obj["type"] === "stdio" && typeof obj["command"] !== "string") {
			throw new Error(`MCP config[${idx}]: stdio config requires a "command" string`);
		}
		if (obj["type"] === "http" && typeof obj["url"] !== "string") {
			throw new Error(`MCP config[${idx}]: http config requires a "url" string`);
		}
