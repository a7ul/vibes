...
export class AGUIAdapter<TDeps, TOutput> {
		...
		const stream = new ReadableStream<Uint8Array>({
			async start(controller) {
				...
				try {
					...
					const initialState = input.state ?? (getState ? await getState() : null);
					...
					for await (const event of eventStream) {
						switch (event.kind) {
							case "turn-start": {
								...
								enqueue({ type: "STEP_STARTED", stepName: `turn-${turnNumber}` });
								break;
							}
							...
							case "tool-call-start": {
								...
								if (currentMessageId !== null) {
									enqueue({ type: "TEXT_MESSAGE_END", messageId: currentMessageId });
							...
							case "usage-update": {
								...
								enqueue({ type: "STEP_FINISHED", stepName: `turn-${turnNumber}` });
								break;
							}
	...
	handler(): (req: Request) => Promise<Response> {
		...
		return async (req: Request): Promise<Response> => {
			...
			return self.handleRequest(input);
		};
	}
}
