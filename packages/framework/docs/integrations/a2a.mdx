...
```mermaid
flowchart TD
    C[Remote Agent / Client]
    C -->|GET /.well-known/agent.json| D[AgentCard Discovery]
    C -->|POST / message/send| E[A2AAdapter]
    C -->|POST / message/stream| E
    C -->|POST / tasks/get| E
...
Install `@vibes/framework` and import `A2AAdapter`:
...
stateDiagram-v2
    ...
    working --> completed : agent finished successfully
    working --> failed : unhandled error
    working --> canceled : tasks/cancel called
    working --> input-required : agent needs clarification
    note right of input-required : not yet fully wired
```
...
```typescript
import { A2AAdapter, MemoryTaskStore } from "npm:@vibes/framework";
