...
sequenceDiagram
    ...
    participant WF as workflowFn
    participant A as activities.runModelTurn
    participant AG as TemporalAgent → Agent

    C->>T: client.workflow.start(workflowFn, { args })
    T->>W: schedule workflow execution
    W->>WF: run workflowFn(prompt)
    WF->>A: schedule runModelTurn activity
    A->>AG: TemporalAgent.run(prompt)
    AG-->>A: ModelTurnResult
    A-->>WF: result (serialized, replay-safe)
    WF-->>T: workflow complete
    T-->>C: handle.result() resolves
```
