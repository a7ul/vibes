...
```mermaid
flowchart TD
    A[Agent] --> B[MCPToolset / MCPManager]
    B --> C[MCPClient interface]
    C --> D[MCPStdioClient\nsubprocess via stdio]
    C --> E[MCPHttpClient\nremote server via HTTP/SSE]

    F[MCPManager] --> G[MCPToolset A]
    F --> H[MCPToolset B]
    G --> I[MCPStdioClient]
    H --> J[MCPHttpClient]
    F --> A
...
```typescript
import { Agent, MCPHttpClient, MCPToolset } from "npm:@vibes/framework";
...
```typescript
import { MCPToolset } from "npm:@vibes/framework";
