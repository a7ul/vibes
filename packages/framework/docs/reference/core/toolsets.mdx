...
> **RunContext flows through toolsets.** Every `tools(ctx)` call receives the full `RunContext` including your `deps`. This is what makes role-based and permission-based tool visibility possible. See [Dependency Injection](../../concepts/dependency-injection) for the full hierarchy.
