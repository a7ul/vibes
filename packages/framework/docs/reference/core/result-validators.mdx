...
> **RunContext flows through validators.** Every validator receives `(ctx, output)` with your injected `deps` - the equivalent of pydantic-ai's `@agent.result_validator`. See [Dependency Injection](../../concepts/dependency-injection) for the full hierarchy.
