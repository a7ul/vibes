...
> **RunContext flows through tools.** `execute`, `prepare`, and `requiresApproval` all receive a `RunContext` with your injected `deps`. This is the equivalent of pydantic-ai's `@agent.tool`. See [Dependency Injection](../../concepts/dependency-injection) for the full picture.
