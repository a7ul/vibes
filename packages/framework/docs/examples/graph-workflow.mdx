...
<Warning>
Always import `next` and `output` as free functions from `@vibes/framework`. Do **not** call them as instance methods (e.g., prefixed with `this.`) - they are standalone functions, not methods on `BaseNode`, and calling them as such will throw at runtime.
</Warning>
