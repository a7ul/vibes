...
1. **System prompts** are resolved (static strings and dynamic functions, all joined)
...
See [How Agents Work](../concepts/how-agents-work) for a deeper dive.

## Dynamic System Prompts

This is one of the most powerful features of Vibes, directly equivalent to pydantic-ai's `@agent.system_prompt` decorator.

System prompts can be functions that receive a `RunContext` at runtime. This lets you inject user data, session state, or anything else into the prompt dynamically:
...
## Adding System Prompts Dynamically

Use `agent.addSystemPrompt()` to attach additional prompts after construction. This is the equivalent of pydantic-ai's `@agent.system_prompt` decorator - perfect for middleware, plugins, or building agents incrementally:
...
// Add a dynamic prompt - called every run with the current context
agent.addSystemPrompt((ctx) => `Today is ${new Date().toDateString()}.`);

// Add a prompt that uses injected dependencies
agent.addSystemPrompt((ctx) => `Current user: ${ctx.deps.username}`);
```

All system prompts - whether passed at construction or added later - are joined with `\n\n` and sent as a single system message each run.

> **pydantic-ai mapping:** `agent.addSystemPrompt(fn)` is the TypeScript equivalent of:
> ```python
> @agent.system_prompt
> def my_prompt(ctx: RunContext[Deps]) -> str:
>     return f"Current user: {ctx.deps.username}"
> ```

## Multiple System Prompts

Combine static and dynamic prompts using both `systemPrompt` and `instructions`:
...
- [Adding Tools](./adding-tools) - give the agent real capabilities
- [Structured Output](./structured-output) - get typed JSON responses
- [Testing Your Agent](./testing) - test without API calls
