---
title: Models
description: Vibes uses the Vercel AI SDK model abstraction, accepting any LanguageModel-compatible provider. Swap models at construction time or override per run.
---
...
graph TD
    ...
    Agent --> LM["LanguageModel interface\n(Vercel AI SDK)"]
    LM --> Anthropic["@ai-sdk/anthropic"]
    LM --> OpenAI["@ai-sdk/openai"]
    LM --> Google["@ai-sdk/google"]
    LM --> Groq["@ai-sdk/groq"]
    LM --> Mistral["@ai-sdk/mistral"]
    LM --> Ollama["ollama-ai-provider"]
    LM --> Compat["@ai-sdk/openai\n(custom baseURL)"]
    Anthropic --> APIs["LLM APIs"]
    OpenAI --> APIs
    Google --> APIs
    Groq --> APIs
    Mistral --> APIs
    Ollama --> Local["Local Ollama server"]
    Compat --> CustomAPI["Custom / self-hosted API"]
```
