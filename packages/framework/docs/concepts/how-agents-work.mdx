...
`instructions` is appended after `systemPrompt`. Both can be strings or functions that receive `RunContext`. Use `systemPrompt` for stable base config and `instructions` for per-run dynamic content.
...
| `UsageLimitError` | Token/request budget hit | Increase limits or add history processors |
| Provider errors | Model API failure | Retry or handle in calling code |

See [Error Handling](./error-handling) for the full pattern guide.
