...
```typescript
import { imageMessage, audioMessage, fileMessage } from "@vibes/framework";
```
...
```mermaid
flowchart LR
    IMG[Image\nstring / Uint8Array / URL] --> IM[imageMessage]
    AUD[Audio\nstring / Uint8Array] --> AM[audioMessage]
    VID[Video\nstring / Uint8Array] --> FM_V[fileMessage\nvideo/* MIME]
    DOC[Document\nstring / Uint8Array] --> FM_D[fileMessage\napplication/pdf etc.]
    IM --> UMM[UserModelMessage]
    AM --> UMM
    FM_V --> UMM
    FM_D --> UMM
    UMM --> MODEL[Model]
...
import { imageMessage } from "@vibes/framework";
...
```typescript
import { audioMessage } from "@vibes/framework";
...
```typescript
import { fileMessage } from "@vibes/framework";
...
```typescript
import { uploadedFileSchema } from "@vibes/framework";
...
sequenceDiagram
    ...
    App->>Agent: agent.run(messages)
    Agent->>Model: send conversation
    Model->>Agent: call screenshotTool(url)
    Agent->>Tool: execute({ url })
    Tool-->>Agent: BinaryContent { type: "binary", data, mimeType }
    Agent->>Model: tool result (binary image)
    Model-->>Agent: final text response
    Agent-->>App: RunResult
```
