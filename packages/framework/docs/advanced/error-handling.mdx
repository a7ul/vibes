...
<Warning>
The correct class name is `UsageLimitError` — **not** `UsageLimitExceededError`. The old `concepts/error-handling` page used the wrong name. Always import from `@vibes/framework` and rely on TypeScript autocomplete to confirm the name.
</Warning>
