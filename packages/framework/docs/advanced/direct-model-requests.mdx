...
```mermaid
flowchart TD
    START([Need model output?]) --> Q1{Tools, outputSchema,\nmulti-turn, validators,\nor deps?}
    Q1 -->|Yes| AGENT[Use agent.run / agent.stream]
    Q1 -->|No| Q2{One-shot generation,\nno tools, simple prompt?}
    Q2 -->|Yes| DIRECT[Use generateText / streamText]
    Q2 -->|No| AGENT

    NOTE["generateText and streamText are from the Vercel AI SDK.\nVibes re-exports nothing — import from 'ai' and '@ai-sdk/anthropic'."]
    DIRECT --- NOTE
```
...
sequenceDiagram
    ...
    App->>Agent: agent.run(prompt)
    Agent->>Model: send with tools + schema
    Model->>Agent: tool call
    Agent->>Agent: execute tool, validate output
    Agent->>Model: tool result
    Model-->>Agent: final response
    Agent-->>App: RunResult { output, messages }

    Note over App,Model: Direct path (one-shot generation)
    App->>SDK: generateText({ model, prompt })
    SDK->>Model: single request
    Model-->>SDK: response text
    SDK-->>App: GenerateTextResult { text }
```
