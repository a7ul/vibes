...
Vibes is a TypeScript agent framework that brings [Pydantic AI](https://ai.pydantic.dev/)'s proven patterns to the TypeScript ecosystem, powered by [Vercel AI SDK](https://sdk.vercel.ai/)'s model layer. It is designed for developers who want type safety, testability, and production-readiness without sacrificing simplicity.
...
`TestModel`, `FunctionModel`, `agent.override()`, and `setAllowModelRequests(false)` are first-class exports, not afterthoughts. Every agent you write is testable without real API calls, from the first line.

### Robust evaluations

Shipping an agent is easy. Knowing whether it got better or worse after a change is hard. Vibes ships a full [evaluation framework](/concepts/evals): define typed datasets, score outputs with built-in scorers or an LLM-as-judge, and run experiments with configurable concurrency and retries. Evals are code - they live in your repo, run in CI, and give you a quantitative signal every time you change a prompt, swap a model, or refactor a tool.
