...
A GitHub Actions workflow runs every day at 9am UTC. It checks whether a new version of `pydantic-ai` has been published to PyPI. If one has, and no open PR already exists for that version, it:
...
4. **Opens a PR** that bumps the tracked version in `.github/pydantic-ai-version.txt` and includes the full porting checklist in the PR body
5. **Assigns the PR to the GitHub Copilot coding agent**, which uses `.claude/vibes-pydantic-porting.md` as its porting guide to attempt an implementation

The PR is the single unit of work: the checklist, implementation, and review all happen there. It auto-closes when merged. The Copilot agent's output is always reviewed by a human before merging; complex architectural changes go through a full design review.
...
To see the full history of porting work, filter GitHub PRs by the [`pydantic-ai-update`](https://github.com/a7ul/vibes/pulls?q=label%3Apydantic-ai-update) label.
