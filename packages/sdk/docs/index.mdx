...
description: "TypeScript agent framework for building production-grade, type-safe AI applications and workflows, the [Pydantic AI](https://ai.pydantic.dev) way, powered by [Vercel AI SDK](https://sdk.vercel.ai)."
---
...
5. **Testing + Evals - the only way to ship AI to production** - Unit-test every agent in CI with `TestModel` and `setAllowModelRequests(false)` (no real API calls). Then go further with typed eval datasets, built-in and LLM-as-judge evaluators, and experiment runners with configurable concurrency. Evals are code - they live in your repo, run in CI, and catch regressions before they reach users.
