...
sequenceDiagram
    ...
    UI->>API: POST with messages
    API->>Agent: stream response
    Agent-->>UI: token stream
    UI->>UI: render tokens as they arrive
    Agent-->>UI: stream complete
```
...
| `agent.stream(prompt, opts?)` | `@vibesjs/sdk` | Runs the agent, returns `{ textStream, partialOutput, ... }` |
