...
**(1)** Each agent run receives a fresh, cache-free copy of the toolset so tool discovery is always recorded exactly once per run (important for Temporal workflow replay determinism). Within a single run, the tool list is cached for `toolCacheTtlMs` milliseconds — 60 seconds by default. For servers whose tools change frequently, lower the TTL or call `toolset.invalidateCache()` to force a re-fetch on the next turn.<br/>
