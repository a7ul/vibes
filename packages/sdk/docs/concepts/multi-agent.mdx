...
sequenceDiagram
    ...
    App->>Orchestrator: run task
    Orchestrator->>Orchestrator: model decides to delegate
    Orchestrator->>Specialist: run sub-task
    Specialist-->>Orchestrator: result
    Orchestrator->>Orchestrator: model generates final answer
    Orchestrator-->>App: final result
```
