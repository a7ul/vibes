...
const agent = new Agent<undefined, z.infer<typeof Schema>>({
  ...
  resultValidators: [
    (ctx, output) => {
      ...
      return output; // return the output to accept it (or return a modified copy)
    },
  ],
});
```
...
| `retryCount` | `number` | Number of result validation retries that occurred |
| `runId` | `string` | Unique identifier for this run |
| `toolMetadata` | `Map<string, Record<string, unknown>>` | Metadata attached by tools during the run, keyed by tool call ID |
...
console.log(result.retryCount);   // number of validation retries
