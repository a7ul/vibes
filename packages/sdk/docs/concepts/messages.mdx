...
const agent = new Agent({
  model,
  historyProcessors: [
    summarizeHistoryProcessor(model, {
      maxMessages: 10,           // default: 20 - summarize when history exceeds this
      summarizePrompt: "Summarize the conversation so far:", // optional custom prompt
    }),
  ],
});
```
...
Sanitize inbound `messageHistory` from untrusted sources before passing it to `agent.run()` or `agent.stream()`. Malformed messages/parts are dropped.

```typescript
import { sanitizeMessages } from "@vibesjs/sdk";

const safeHistory = sanitizeMessages(untrustedHistory);
