...
sequenceDiagram
    ...
    App->>Agent: stream events
    Agent-->>App: turn started
    Agent->>Model: generate (streaming)
    Model-->>Agent: token stream begins
    Agent-->>App: text delta
    Agent-->>App: text delta
    Agent-->>App: tool call started
    Note over Agent: tool executes
    Agent-->>App: tool call result
    Agent-->>App: usage update
    Agent->>Model: next turn with tool result
    Agent-->>App: turn started
...
| `"tool-call-start"` | `toolName, toolCallId, args` | Model requests a **function** tool call |
| `"tool-call-result"` | `toolCallId, toolName, result` | **Function** tool finished executing |
| `"output-tool-call-start"` | `toolName, toolCallId, args` | Model requests an **output** tool call (`final_result` or `outputTool()`) |
| `"output-tool-call-result"` | `toolCallId, toolName, result` | **Output** tool finished executing |
...
<Note>
Function tools (defined with `tool()`) emit `tool-call-start` / `tool-call-result`.
Output tools (`final_result` and tools defined with `outputTool()`) emit
`output-tool-call-start` / `output-tool-call-result`. This mirrors the
`OutputToolCallEvent` / `OutputToolResultEvent` distinction introduced in Pydantic AI v1.93.0.
</Note>
