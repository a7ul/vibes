...
```mermaid
flowchart TD
    A[Model Requests Tool Call] --> B[Validate Arguments]
    B -- invalid --> C[Return Error to Model]
    B -- valid --> D{Approval Required?}
    D -- yes --> E([Pause for Human Approval])
    D -- no --> F[Execute Tool]
    F -- error --> G{Retries left?}
    G -- yes --> F
    G -- no --> H([Propagate Error])
    F -- success --> I{Terminal Tool?}
    I -- yes --> J([End Run with Output])
    I -- no --> K[Append Result to History]
    K --> L[Continue to Next Turn]
```
