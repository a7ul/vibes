---
title: Testing
description: First-class testing with no real API calls - TestModel, FunctionModel, and setAllowModelRequests(false) make every agent fully testable in CI.
---
...
sequenceDiagram
    ...
    Test->>Agent: run with test model
    Agent->>TestModel: generate turn 0
    TestModel-->>Agent: simulated tool calls
    Agent->>TestModel: generate turn 1
    TestModel-->>Agent: final test response
    Agent-->>Test: Result
```
...
```typescript
import { setAllowModelRequests, TestModel, createTestModel } from "@vibesjs/sdk";
import { z } from "zod";
...
When you need precise control over what the model returns on each turn, use `FunctionModel`. Its constructor takes a function that receives `{ messages, tools, turn }` and returns a `DoGenerateResult`.
