...
sequenceDiagram
    ...
    App->>Agent: run task
    Agent->>Agent: model requests sensitive tool
    Agent-->>App: Approval Required
    App->>Human: show approval UI
    Human-->>App: approved
    App->>Agent: resume with approval
    Agent->>Agent: execute tool, continue run
    Agent-->>App: Result
```
...
```typescript
import { Agent, ApprovalRequiredError, tool } from "@vibesjs/sdk";
...
```typescript
import { Agent, ApprovalRequiredError, ExternalToolset } from "@vibesjs/sdk";
import { anthropic } from "@ai-sdk/anthropic";
