...
Vibes normalizes consecutive leading `system` messages into a single request
message before each model call. This keeps custom OpenAI-compatible backends
happy even when they only accept one initial system message.
