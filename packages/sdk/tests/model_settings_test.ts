...
Deno.test("modelSettings - temperature is passed to generateText", async () => {
  ...
  const model = new MockLanguageModelV3({
    doGenerate: (opts) => {
      capturedTemperature = (opts as Record<string, unknown>).temperature as
        | number
        | undefined;
...
Deno.test("modelSettings - seed is passed to generateText", async () => {
  ...
  const model = new MockLanguageModelV3({
    doGenerate: (opts) => {
      capturedSeed = (opts as Record<string, unknown>).seed as
        | number
        | undefined;
      return Promise.resolve(textResponse("ok"));
    },
  ...
  const model = new MockLanguageModelV3({
    doGenerate: (opts) => {
      capturedTemperature = (opts as Record<string, unknown>).temperature as
        | number
        | undefined;
      capturedTopP = (opts as Record<string, unknown>).topP as
        | number
        | undefined;
      return Promise.resolve(textResponse("ok"));
    },
